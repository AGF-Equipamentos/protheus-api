  ...
  async index(req, res) {
    ...
    await request.query(
      `
            ...
            FROM	  SD4010 AS SD4 WITH (READPAST) INNER JOIN
                    SB1010 AS SB1 WITH (READPAST) ON SB1.D_E_L_E_T_ = '' AND SB1.B1_COD = SD4.D4_COD AND LEFT(SB1.B1_FILIAL, 2) = LEFT(SD4.D4_FILIAL, 2) LEFT OUTER JOIN
                    SC2010 AS SC2 WITH (READPAST) ON SC2.D_E_L_E_T_ = '' AND SC2.C2_FILIAL = SD4.D4_FILIAL AND SC2.C2_NUM + SC2.C2_ITEM + SC2.C2_SEQUEN = SD4.D4_OP
